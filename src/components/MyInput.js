...
const MyInput = (props) => {
  ...
  return (
    <SafeAreaView >
     ...
     <Text style={[styles.labelText,props.changeBcColor=='white'?{color:'#11141C'}:{color:colors.input_text}]}>{props.label}</Text>
          <Controller
         ...
         numberOfLines = { props?.textArea && 4}
              ...
              returnKeyLabel="go"
                ...
                style={[styles.input_style, {   height: props.textArea?100: Platform.OS == 'ios' ? 57 : 58 , textAlignVertical: 'top',paddingTop:15, },props.changeBcColor=='white'?{color:'#11141C'}:{color:colors.input_text}]}
                inputContainerStyle={[styles.inputContainerStyle,{paddingTop:props.textArea&&-100,},props.changeBcColor=='black'?{ borderColor:isFocused[name]?'white':'#D3D3D3'}:{ borderColor:isFocused[name]?'black':'#D3D3D3'}]}
                value={value}
                placeholder={props.placeholder}
                autoCapitalize="none"
                placeholderTextColor={props?.changeBcColor=='black'?'white': colors.input_text_focus}
