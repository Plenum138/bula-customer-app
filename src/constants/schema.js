...
export const createOrderSchema = yup
  .object({
    ...
    customer_name: yup
      ...
      .matches(
        /^[a-zA-Z]+(?:[\s.]+[a-zA-Z]+)*$/,
        'First and last space, dots,numbers are not allowed',
      ),
    method: yup.string().required('method type is required').nullable(),
    address: yup.string().required(),
  })
  .required();
export const searchAndInputSchema = yup
  .object({
  ...
  .required();
export const EditOrderRbSheetSchema = yup
  .object({
    productName: yup
      .string()
      .required('Customer name  is Required ')
...
export const createProductSchema = yup
  .object({
    productName: yup
      ...
      .max(25)
      .min(3)
      .matches(/^[^\s]+(\s+[^\s]+)*$/, 'First and last space are not allowed'),
    category: yup
      .string()
      .required()
      .max(25)
  ...
  const createProductSchema = yup
    .object({
      productName: yup
        ...
        .matches(
          /^[^\s]+(\s+[^\s]+)*$/,
          'First and last space are not allowed',
        ),
      category: yup
        .string()
        .required()
...
export const AddCustomerSchema = yup
  .object({
    customer_name: yup
      ...
      .required('Customer name  is Required ')
      .max(25)
      .min(3, 'Mobile Number must be at least 3 characters')
      .matches(
        /^[a-zA-Z]+(?:[\s.]+[a-zA-Z]+)*$/,
        'First and last space, dots,numbers are not allowed',
      ),
